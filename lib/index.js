...
const internals = {
  schema: Joi.object({
    ...
    migrations: Joi.object({
      auto: Joi.boolean().truthy(['yes', 'true', '1', 1]).falsy('no', 'false', '0', 0).insensitive(),
...
internals.provision = async function (opts) {
  ...
  const settings = Joi.attempt(opts, internals.schema, 'Invalid database configuration');
  Hoek.assert(!this.has(settings.name), `Database connection "${settings.name}" already exists`);

  const knexOpts = Hoek.cloneWithShallow(settings, [
...
internals.snakeCaseMappers = () => ({
  parse: internals.keyMapper(internals.memoize(camelCase)),
  format: internals.keyMapper(internals.memoize(snakeCase))
});
